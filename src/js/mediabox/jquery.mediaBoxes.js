 ...
 * ====================================================================== */
				...
				container
					...
					.each(function () {
						...
						if (thumbnailDiv[0] != undefined && image.hasClass("done")) {
							var thumbnailDiv_ratio =
								parseFloat(thumbnailDiv.data("width")) /
								parseFloat(thumbnailDiv.data("height"));
							var image_ratio =
			...
			container
				...
				.each(function () {
					...
					if (imagesWithDimensions == true) {
						...
						thumbnailDiv.prepend(
							'<div class="media-boxes-loading-animation"></div>'
						);
     ...
     * ====================================================================== */
		...
		filtersContainers.on("click", "*[data-filter]", function (e) {
			if (!$(this).is("option") && !$(this).is(":checkbox")) {
				// check if its not an option from a select element or if its not a checkbox
				/* current items */
		...
		$("body").on("click", settings.filterContainer, function () {
			if ($(this).is(":checkbox")) {
				...
				if (current_filter.is(":checked")) {
					current_filter.attr(
						"data-filter",
						current_filter.attr("data-filter-original")
					);
				} else {
		...
		filtersContainers.each(function () {
			...
			if ($(this).is(":checkbox")) {
				...
				current_filter.attr(
					...
					current_filter.attr("data-filter")
				); /* save original data-filter in another attribute */
						...
						if (!item.is(":checkbox") && !item.is("select")) {
							if (item.hasClass("selected")) {
								...
								if ($this.hasClass("selected")) {
									$this
										.parents(settings.filterContainer)
										.find('[data-filter="*"]')
										.trigger("click");
								}
							}
     ...
     * ====================================================================== */
		...
		function setupOverlayForHoverEffect(container) {
			...
			boxes.each(function () {
				...
				container
					.data("settings")
					.animation_on_thumbnail_overlay_hover.forEach(function (row) {
...
*/
     ...
     * ====================================================================== */
		...
		$("body").on("click", "div.media-boxes-pinterest", function () {
			...
			var url = encodeURIComponent($this.data("url"));
			url =
				"http://pinterest.com/pin/create/button/?url=" + url + "&media=" + url;
			openWindow(url);
		});
